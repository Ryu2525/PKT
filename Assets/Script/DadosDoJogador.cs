...
public static class DadosDoJogador
{
    ...
    public static bool VitoriaDefinitiva = false;
    ...
    public static float Fama = 0.25f;
    public static float MoralTreinador = 0.4f;
    public static float MoralApostador = 0.2f;
    public static float RiscoInterrogatorio = 0f;
    ...
    // Mude de 'private' para 'public' para o Manager poder usar
    public static void ExecutarFracassoOuRebaixamento()
    {
        if (NivelAtual == 1)
        {
        ...
        else
        {
            ...
            CiclosFracassados = 0;
            AtualizarDadosDoTime();
            SceneManager.LoadScene("Hub"); // Adicione isso para ele voltar ao Hub após cair de nível
        }
    }
    ...
    private static void ConcluirCicloDePartidas()
    {
        ...
        if (VitoriasNoCiclo >= 2)
        {
            ...
            Debug.Log("===== VERIFICAÇÃO DE VITÓRIA FINAL =====");
            Debug.Log("Nível Atual: " + NivelAtual);
            Debug.Log("Vitórias no Ciclo: " + VitoriasNoCiclo + "/3");
            Debug.Log("Felicidade: " + Felicidade);
            Debug.Log("Fama: " + Fama);
            ...
            {
                VitoriaDefinitiva = true;

                Debug.Log("RESULTADO: JOGADOR CONSEGUIU A VITÓRIA DEFINITIVA!");
            }
    ...
    private static void ProcessarFracassoCritico()
    {
        if (NivelAtual == 1)
        {
            // Alterado de "GameOver" para "FimFase1"
            SceneManager.LoadScene("FimFase1");
        }
        else
        {
    ...
    public static void LimitarBarras()
        {
            ...
            RiscoInterrogatorio = Mathf.Clamp01(RiscoInterrogatorio);
    ...
    public static void VerificarFimDeJogoPorBarras()
    {
        LimitarBarras();

        if (Felicidade <= 0f)
        {
            Debug.Log("Fim de jogo: felicidade chegou a zero.");
            // Alterado de "Home" para "FimDeCarreira"
            SceneManager.LoadScene("FimDeCarreira"); 
            return;
        }

        if (MoralTreinador <= 0f)
        {
            Debug.Log("Fim de jogo: moral com o treinador chegou a zero.");
            // Alterado de "Home" para "FimDeCarreira"
            SceneManager.LoadScene("FimDeCarreira");
            return;
        }
